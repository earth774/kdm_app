...
app.controller("ImporterCtrl", function ($scope, $http, $cookies, $compile, $filter, NgMap) {
    var province_id;
    var district_id;
    var sub_district_id;
    ...
    $scope.clearData = function () {
        ...
        $('#select_country').val("");
        $('#select_import_cost_type_id').val("");
    ...
    function checkselectoption() {
        ...
        if ($scope.dataimport.product_info_id != null) {
            initMaterialSelect('#select_trading_mode', $scope.dataimport.product_info_id);
        }

        if ($scope.dataimport.source_country_id != null) {
            initMaterialSelect('#select_country', $scope.dataimport.source_country_id);
        }

        if ($scope.dataimport.import_cost_type_id != null) {
            initMaterialSelect('#select_import_cost_type_id', $scope.dataimport.import_cost_type_id);
        }
    }
    // --------------------map-------------------------
    ...
    // ประเภทค่าขนส่ง
    $scope.getImportCostType = function () {
        $http({
            method: 'GET',
            url: url + '/import_cost_type',
            dataType: "json",
            headers: {'Content-Type': 'application/json', 'Authorization': $cookies.get('api_token')}
        }).then(function onSuccess(response) {
            var res = response.data;
            $scope.import_cost_type = res.data;
            if (res.data.length == 0) {
                setTimeout(function () {
                    initMaterialSelect('#select_import_cost_type_id');
                }, 500);
    ...
    // =================function=======================
    // ข้อมูส่วนตัว
    $scope.funcGetImporterByID = function () {/*{{host}}\importer_info\user_id\1*/
        $http({
            method: 'GET',
            url: url + '/importer_info/user_id/' + $scope.user_id,
            dataType: "json",
            headers: {'Content-Type': 'application/json', 'Authorization': $cookies.get('api_token')}
        }).then(function onSuccess(response) {
            var res = response.data;
    ...
    $scope.funcInsertImporter = function (require) {/*insert importer*/
        if(!require){
            $http({
                method: 'POST',
                url: url + '/importer_info',
    ...
    // อัพเดทข้อมูล
    $scope.funcUpdateImporter = function (require) {

    if(!require){
        $http({
            method: 'PUT',
            url: url + '/importer_info/' + $scope.user_id,
            dataType: "json",
            data: $scope.user_data,
            headers: {'Content-Type': 'application/json', 'Authorization': $cookies.get('api_token')}
        }).then(function onSuccess(response) {
    ...
    $scope.funcGetBuyingItemByID = function () {
        ...
        $scope.ajaxImportOptions = {
            "ajax": {
                "url": url + '/import_item_info/user_id/' + $scope.user_id + '?user_type_id=' + user_type_id,
                "beforeSend": function (request) {
                    request.setRequestHeader("Authorization", $cookies.get('api_token'));
                }
            },
            ...
            "fnRowCallback": function (nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                ...
                $('td', nRow).eq('3').html($filter('limitTo')(aData.import_datetime, 10, 0));
                $(nRow).attr("id", 'row_' + aData.id);
                return nRow;
            },
            "fnDrawCallback": function (oSettings, json) {
        ...
        setTimeout(function () {
            $('#table_data_import').DataTable().ajax.reload();
        }, 300);
    }

    $scope.funcInsertImportItem = function (require) { /*เพิ่มข้อมูลสินค้านำเข้า*/
            if(!require){
                $http({
                    method: 'POST',
                    url: url + '/import_item_info',
                    dataType: "json",
    ...
    $scope.funcUpdateImportItem = function (require) { /*แก้ไขข้อมูลสินค้านำเข้า*/
            if(!require){
                $http({
                    method: 'PUT',
                    url: url + '/import_item_info/' + $scope.dataimport.id,
                    dataType: "json",
                    data: $scope.dataimport,
                    headers: {'Content-Type': 'application/json', 'Authorization': $cookies.get('api_token')}
                }).then(function (res) {
    ...
    $scope.detailImportItem = function (import_item_id) {
        ...
        $http({
            method: 'GET',
            url: url + '/import_item_info/' + import_item_id,
            dataType: "json",
            headers: {'Content-Type': 'application/json', 'Authorization': $cookies.get('api_token')}
        }).then(function onSuccess(response) {
            var res = response.data;
            $scope.dataimport = res.data[0];
            $scope.dataimport.import_datetime = $filter('limitTo')($scope.dataimport.import_datetime, 10, 0);
    ...
    $scope.DeleteData = function(user_id){
        $http({
            method: 'DELETE',
            url: url + '/importer_info/user_id/'+ user_id,
            dataType: "json",
            headers: {'Content-Type': 'application/json','Authorization': $cookies.get('api_token')}
        }).then(function onSuccess(response) {
    ...
    $scope.delData = function (obj) {
        ...
        $http({
            method: 'DELETE',
            url: url + '/import_item_info/' + obj.id,
            dataType: "json",
            headers: {'Content-Type': 'application/json', 'Authorization': $cookies.get('api_token')}
        }).then(function onSuccess(response) {
            var res = response.data;
