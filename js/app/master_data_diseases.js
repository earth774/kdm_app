...
app.controller("MasterDataDiseasesCtrl", function ($scope, $http, $cookies, $compile) {
    $scope.div_table_data_diseases = 1;
    $scope.btn_menu_add = 1;
    $scope.div_add_data_diseases = 0;
    $scope.form_cmd = "";
        ...
        $scope.ajaxOptions = {
            ...
            "fnRowCallback": function( nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                    var html = //'<a class="btn-floating btn-sm blue lighten-3 waves-effect waves-light" data-toggle="modal" data-target="#ProductDetail" ng-click="ProductDetailInfo('+aData.id+');"><i class="fa fa-eye"></i></a>'
                           '<a class="btn-floating btn-sm blue light-blue darken-4 waves-effect waves-light" ng-click="DiseasesDetailInfo('+aData.id+',\'edit\');"><i class="fa fa-pencil"></i></a>'
    ...
    $scope.DeleteData = function(id){
        $http({
            ...
            if(data.status == "error"){
                ...
                $('#table_data_diseases').DataTable().ajax.reload();
            }
    ...
    // form
    var $form_add_data_diseases = $("#form_add_data_diseases");
    $form_add_data_diseases.validate({
    ...
    $scope.showFormAddProduct = function(cmd){ //show form
        ...
        $scope.div_table_data_diseases = 0;
        $scope.div_add_data_diseases = 1;
    ...
    function checkselectoption() {
        // $scope.breed_info_id = global_breed_info_id;
    ...
    $scope.SubmitForm = function(id){ //submit
        if($form_add_data_diseases.validate().form()){
    ...
    //Edit
    $scope.DiseasesDetailInfo = function(id){
        $http({
            method: 'GET',
            url: url + '/diseases_info/'+id,
            dataType: "json",
            ...
            if(data.status == "error"){
                ...
                global_breed_info_id = ret.breed_info_id;
                ...
                if($scope.breed_info_id != 0 || $scope.breed_info_explanation != null){
                    $http({
                        ...
                        data: { "name" : $scope.name,
                                ...
                                'breed_info_explanation': $scope.breed_info_explanation
                                }, //forms user object
                        headers: {'Content-Type': 'application/json','Authorization': $cookies.get('api_token')}
                    }).then(function onSuccess(response) {
                        var data = response.data;
